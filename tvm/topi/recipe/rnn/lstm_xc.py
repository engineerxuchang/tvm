...
def lstm():
    ...
    s_init_c = tvm.compute((1, batch_size, num_hidden),
                           lambda *i: 0.0, name="init_c")   # 初始c状态为0
    s_init_h = tvm.compute((1, batch_size, num_hidden),
                           lambda *i: 0.0, name="init_h")   # 初始h状态为0
    # LSTM transition
    ...
    update_c = tvm.compute(gshape, lambda *i: next_c(*i), name="update_c")  # 更新细胞状态
    update_h = tvm.compute(gshape, lambda *i: next_h(*i), name="update_h")  # 更新输出
